 ...
 */
	public function __construct(View $View, $settings = []) {
		$this->settings = Hash::merge($this->__defaultSettings, $settings);
 ...
 */
	public function head($columns = [], $rowOptions = [], $options = []) {
 ...
 * @return null|string
 */
	public function fallback($columns, $options = []) {
 ...
 */
	public function foot($columns = [], $rowOptions = [], $options = []) {
		$rowOptions = Hash::merge($this->settings['rowOptions'], $rowOptions);
 ...
 */
	public function cell($value, $key = null, $tag = 'td', $count = false) {
		...
		return $this->Html->tag(
			...
			Hash::merge($this->settings['cellOptions'], is_array($value) && array_key_exists(1, $value) ? $value[1] : [])
		);
	}
 ...
 */
	private function __row($columns = [], $options = [], $tag = 'td') {
		$this->resetCellCount();
		$cells = null;
